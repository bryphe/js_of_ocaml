...
let f
    ...
    then
      warn
        "Warning: '--source-map' is enabled but the bytecode program was compiled with \
         ...
         %!"
  in
  let pseudo_fs kind debug cmis =
    let cmis = if nocmis then StringSet.empty else cmis in
    let paths = paths @ StringSet.elements (Parse_bytecode.Debug.paths debug ~units:cmis) in
    PseudoFs.f kind cmis fs_files paths
  in
  let env_instr =
  ...
  in
  let fs_external_instr =
    if fs_external
    then [Code.(Let (Var.fresh (), Prim (Extern "caml_fs_init", [])))]
    else []
  in
  let output (one : Parse_bytecode.one) standalone output_file =
    ...
    (match output_file with
     | None ->
       let instr =
         List.concat
           [ pseudo_fs `Inside one.debug one.cmis
           ; env_instr
           ; fs_external_instr ]
       in
     ...
     | Some file ->
       gen_file file (fun chan ->
         let fs_instr1, fs_instr2 =
           ...
           | None -> pseudo_fs `Inside one.debug one.cmis, []
           | Some _ -> [], pseudo_fs `Outside one.debug one.cmis
         in
         let instr =
           List.concat
             [ fs_instr1
         ...
         Option.iter fs_output ~f:(fun file ->
           gen_file file (fun chan ->
             ...
             Driver.f ~standalone ?profile ?custom_header ~global pfs_fmt one.debug code ) )));
    if times () then Format.eprintf "compilation: %a@." Timer.print t
  in
  (match result with
   | Standalone code ->
     output code true output_file
   | Partial code ->
