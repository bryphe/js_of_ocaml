...
module Share = struct
  ...
  let add_args args t =
    List.fold_left args ~init:t ~f:(fun t a ->
        match a with Pc c -> get_constant c t | _ -> t)
...
let kind k =
  match k with `Pure -> const_p | `Mutable -> mutable_p | `Mutator -> mutator_p
    ...
    let rec loop low up =
      ...
      then Branch (fst array_norm.(0))
        ...
        try
          ...
          | [|(b1, [i1]); (b2, _l2)|] -> If (CEq (Int32.of_int i1), Branch b1, Branch b2)
          | [|(b1, _l1); (b2, [i2])|] -> If (CEq (Int32.of_int i2), Branch b2, Branch b1)
          | [|(b1, l1); (b2, l2)|] ->
              let bound l1 =
...
let rec translate_expr ctx queue loc _x e level : _ * J.statement_list =
  ...
  | Closure (args, ((pc, _) as cont)) ->
      ...
      let clo = match clo with (st, J.N) :: rem -> (st, J.U) :: rem | _ -> clo in
      ...
      let res =
        ...
        | Extern "caml_js_var", [Pc (String nm | IString nm)]
         |Extern ("caml_js_expr" | "caml_pure_js_expr"), [Pc (String nm | IString nm)]
          -> (
          try
            let offset = match loc with J.N | J.U -> None | J.Pi pi -> Some pi in
        ...
        | Extern "caml_alloc_dummy_function", [_; size] ->
            let i, queue =
              ...
              match cx with J.ENum i -> Int32.of_float i, queue | _ -> assert false
            in
            let args =
              Array.to_list
            ...
            let call =
              J.ECall
                (J.EDot (J.EVar f, "fun"), List.map args ~f:(fun v -> J.EVar v), loc)
            in
            let e =
              J.EFun
...
and translate_instr ctx expr_queue loc instr =
  ...
  | Let (x, e) -> (
      ...
      | 1, _
        when Config.Flag.compact ()
             && ((not (Config.Flag.pretty ())) || not (keep_name x)) ->
  ...
  then continuation queue
    ...
    | Some (x0, (h_pc, h_args)) ->
        ...
        let rec loop continuation old args params queue =
          ...
          | [], [] -> continuation queue
          | x :: args, y :: params ->
              let z, old = match old with [] -> None, [] | z :: old -> Some z, old in
