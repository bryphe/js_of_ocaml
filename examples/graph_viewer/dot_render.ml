...
let f g =
  ...
  let margin =
    try parse_float (StringMap.find "margin" g.G.graph_attr) with Not_found -> 4.
  in
  ...
  IntMap.iter
    (fun _ n ->
      ...
      in
      (*XXX parse style *)
      let style = try StringMap.find "style" n.G.node_attr with Not_found -> "" in
      let fillcolor =
        if style <> "filled"
        then None
        else
      ...
      | "box" | "rect" | "rectangle" ->
          ...
          Scene.add
            scene
            (Scene.rectangle (x -. w2, y -. h2, x +. w2, y +. h2) fillcolor color)
      | _ ->
          Scene.add
            scene
